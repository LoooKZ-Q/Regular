...
if __name__ == '__main__':
    ...
    # lab = tkinter.Label(top, text='匹配正则表达式内容', width=15, height=2)
    # lab.pack()  # 放置
    ...
    button_match = tkinter.Button(top, text='match', width=15, height=2, command=is_match).grid(row=1, column=2)  # 创建按钮，选定位置

    button_search = tkinter.Button(top, text='search', width=15, height=2, command=is_search).grid(row=2, column=2)

    button_findall = tkinter.Button(top, text='findall', width=15, height=2, command=is_findall).grid(row=3, column=2)
